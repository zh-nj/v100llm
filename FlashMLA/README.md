...
- SM90 / SM100, plus opt-in SM70 preview paths (See the support matrix below)
...
| Dense Decoding | SM70 preview | MQA | FP16 only |
| Sparse Decoding | SM70 preview | MQA | V32/MODEL1 FP8 alpha |
| Sparse Prefill | SM70 preview | MQA | BF16 online MMA_884 alpha |
...
SM70 preview support is opt-in and targets Volta/V100. Build with `FLASH_MLA_ENABLE_SM70=1` and a CUDA 12.x toolchain that can still emit `sm_70`; on CUDA 12.8 also set `FLASH_MLA_DISABLE_SM100=1` unless SM100 compilation is available. The current SM70 path is correctness-first, not a Hopper-performance target. Dense decode is FP16-only and defaults to `FLASH_MLA_SM70_H_TILE=4`, `FLASH_MLA_SM70_CTA_THREADS=256`; these build-time tuning knobs can be overridden before rebuilding. Sparse decode currently supports V32 and MODEL1 FP8 KV cache alpha paths (`D_qk=576/512`, `H_q=64/128`, `topk + extra_topk <= 8192`, optional `topk_length`, optional `extra_k_cache`, optional `extra_topk_length`, optional `attn_sink`) and defaults to `FLASH_MLA_SM70_SPARSE_DECODE_CTA_THREADS=256`, `FLASH_MLA_SM70_SPARSE_DECODE_K_TILE=32`, and `FLASH_MLA_SM70_SPARSE_DECODE_USE_MMA_884_ONLINE=1`; the online path keeps split-K/combine and supports the `max_topk` smoke in `benchmark/bench_sm70_sparse_decode.py --cases max_topk`. Sparse prefill supports BF16 `D_qk=512/576`, `D_v=512`, `H_q=64/128`, optional `topk_length`, optional `attn_sink`, and `topk <= 8192`; it defaults to `FLASH_MLA_SM70_SPARSE_PREFILL_CTA_THREADS=256`, `FLASH_MLA_SM70_SPARSE_PREFILL_K_TILE=32`, and `FLASH_MLA_SM70_SPARSE_PREFILL_USE_MMA_884_ONLINE=1`, with `=0` retaining the warp-SIMT shared-staging fallback.

SM70 regression commands:

```bash
FLASH_MLA_ENABLE_SM70=1 FLASH_MLA_DISABLE_SM100=1 python -m pip install -v . --no-build-isolation
CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=2,3,4,5 PYTHONPATH=/mnt/data/apps/FlashMLA/build/lib.linux-x86_64-cpython-313 python tests/test_flash_mla_dense_decoding.py --dtype fp16
CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=2,3,4,5 PYTHONPATH=/mnt/data/apps/FlashMLA/build/lib.linux-x86_64-cpython-313 python tests/test_flash_mla_sparse_decoding.py
CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=2,3,4,5 PYTHONPATH=/mnt/data/apps/FlashMLA/build/lib.linux-x86_64-cpython-313 python tests/test_flash_mla_sparse_prefill.py
CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=2,3,4,5 PYTHONPATH=/mnt/data/apps/FlashMLA/build/lib.linux-x86_64-cpython-313 python benchmark/bench_sm70_dense_decode.py --cases long --warmup 3 --runs 5
CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=2,3,4,5 PYTHONPATH=/mnt/data/apps/FlashMLA/build/lib.linux-x86_64-cpython-313 python benchmark/bench_sm70_dense_decode.py --cases tile --warmup 3 --runs 5
CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=2,3,4,5 PYTHONPATH=/mnt/data/apps/FlashMLA/build/lib.linux-x86_64-cpython-313 python benchmark/bench_sm70_sparse_decode.py --cases quick --warmup 0 --runs 1
CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=2,3,4,5 PYTHONPATH=/mnt/data/apps/FlashMLA/build/lib.linux-x86_64-cpython-313 python benchmark/bench_sm70_sparse_decode.py --cases max_topk --warmup 0 --runs 1
CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=2,3,4,5 PYTHONPATH=/mnt/data/apps/FlashMLA/build/lib.linux-x86_64-cpython-313 python benchmark/bench_sm70_sparse_prefill.py --cases quick --warmup 0 --runs 1
```
