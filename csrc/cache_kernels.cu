...
// Each entry copies sizes[i] bytes from raw pointer src_ptrs[i] to dst_ptrs[i].
// On CUDA 12.8+ this uses cuMemcpyBatchAsync (resolved at runtime so binaries
// still load on older drivers); otherwise it falls back to a loop of
// cudaMemcpyAsync. Backported from upstream vLLM v0.22.0.
void swap_blocks_batch(const torch::Tensor& src_ptrs,
  ...
  // int64_t and CUdeviceptr/void*/size_t are all 8 bytes on 64-bit platforms,
...
#endif
  {
    // Fallback for CUDA < 12.8 and older CUDA drivers: individual async
    // copies. cudaMemcpyDefault lets the driver infer direction from pointer
    // types.
