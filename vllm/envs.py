...
if TYPE_CHECKING:
    ...
    # this flag only relaxes the caller-side gate. Default ON since H75
    # validation: MTP=1 TPOT 62.4->34.5 ms at 12k and beats MTP=0 at
    # every ctx; acceptance rate, KV memory, and semantic capability
    # (arith + needle vs MTP=0 control) all unchanged. Set to 0 to fall
    # back to the legacy paged kernel for MTP decodes.
    VLLM_V4_MTP_LONG_CTX_FIX: bool = True
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    #
    # H75 R3 (REVERTED): the default was briefly raised to 256 for
    # throughput (TTFT -20%, byte-equivalent short canary). But the H75
    # needle-topk sweep (measurements/h75_needle_topk_dependency.md)
    # showed topk=256 halves long-range single-fact recall (needle
    # retrieval ceiling ~2k vs ~5k raw tokens at topk=512). Default
    # restored to 512 (the model config ceiling for V4-Flash) to keep
    # long-context recall intact by default. Operators who want the
    # throughput tradeoff can opt in with VLLM_DEEPSEEK_V4_INDEXER_TOPK=256.
    # Setting "0" explicitly falls back to model config for back-compat.
    "VLLM_DEEPSEEK_V4_INDEXER_TOPK": lambda: int(
        os.getenv("VLLM_DEEPSEEK_V4_INDEXER_TOPK", "512")
    ),
    ...
    # to serve next_n == 2 (MTP=1). Default ON since H75 validation; see
    # envs hint above. Set VLLM_V4_MTP_LONG_CTX_FIX=0 to fall back.
    "VLLM_V4_MTP_LONG_CTX_FIX": lambda: bool(
        int(os.getenv("VLLM_V4_MTP_LONG_CTX_FIX", "1"))
    ),
    ...
    # When TileLang sparse prefill has a cached bf16-output kernel, pass
    # model fp16 Q directly and keep only the bf16 output staging. Set to 0
    # to restore the older fp16->bf16 input trampoline for debugging.
    ...
    # Peak sparse-indexer logits workspace to reserve during profiling, in MiB.
