...
def flash_mla_with_kvcache_fp8(*args, **kwargs) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Compatibility wrapper for callers that still use the older FP8-specific
    entrypoint name.
    """
...
def flash_mla_sparse_fwd(
    ...
    H15: if `VLLM_SM70_USE_TILELANG_SPARSE_PREFILL=1`, dispatches to
    the TileLang kernel that batches all h_q query heads per block
    (~3x faster than the FlashMLA SM70 path at DSv4F production
    shape). Falls back to the FlashMLA C++ path otherwise.
    """
    try:
        import vllm.envs as envs
        use_tilelang = getattr(envs, "VLLM_SM70_USE_TILELANG_SPARSE_PREFILL", False)
    except Exception:
        use_tilelang = False
    ...
    if use_tilelang:
        try:
            ...
            if ok:
                ...
                import warnings
                warnings.warn(
                    f"VLLM_SM70_USE_TILELANG_SPARSE_PREFILL=1 but tilelang "
                    f"is not available ({reason}); falling back to FlashMLA."
                )
        except ImportError:
            pass  # fall through to FlashMLA path
